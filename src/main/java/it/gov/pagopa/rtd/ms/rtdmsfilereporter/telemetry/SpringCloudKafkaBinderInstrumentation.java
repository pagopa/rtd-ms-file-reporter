package it.gov.pagopa.rtd.ms.rtdmsfilereporter.telemetry;
 ...
 * <p>
 * The `binderCustomizer` method configures the Kafka binder to be traced with opentelemetry by
 * setting a custom client factory and by customizing the kafka container. The
 * `customizedKafkaClient` wrap the producer using opentelemetry instrumentation kafka library,
 * while `containerCustomizer` customize the container (consumer) using a spring kafka opentelemetry
 * library
 * <p>
